...
def d_nsvd(matrix, rank=1):
    ...
    #print(torch.sum(torch.isnan(S)), matrix.size())
    return U, S, V
...
class SVD_Linear(nn.Module):
    ...
    def __init__(self, in_features, out_features, bias, device=None, dtype=torch.double, rank=1):
        ...
        factory_kwargs = {'device': device, 'dtype': dtype}
        self.fc_V = nn.Linear(in_features, rank, False)#.double()
        self.vector_S = nn.Parameter(torch.empty((1, rank), **factory_kwargs))#.to('cuda:2').double() #+ 1e-10
        #self.vector_S = nn.Parameter(torch.randn(1,rank))
        self.fc_U = nn.Linear(rank, out_features, bias)#.double()
