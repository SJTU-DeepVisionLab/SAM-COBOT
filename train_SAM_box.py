...
def main():
    ...
    parser.add_argument("--save_path", type=str, help="save path", default='Adapter_relation_Mobile_20EPOCH')
    ...
    print_trainable_parameters(net.sam)
    #ipdb.set_trace()
        ...
        for step, (image, box_mask_pairs) in enumerate(tqdm(train_loader)):
            ...
            for idx in range(len(box_mask_pairs)):
                ...
                for i in range(len(boxes_item)):
                    box = boxes_item[i]
                    box = box[:4]
                    jitter = np.random.randint(low=0, high=50, size=4)
                    box += jitter    
